...
const CreateBlogPage = () => {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post("http://localhost:8000/user/blog", {
  ...
  return (
    <div className={"flex flex-col justify-center overflow-y-auto"}>
      ...
      <div className="flex flex-row gap-3 justify-end mr-[5vw] h-[40px] text-[18px] m-6 mt-20">
        ...
        <button className={"text-white bg-[#16C47F] p-4 rounded-lg flex items-center font-bold w-[200px] justify-center"} onClick={handleSave}>Publish Blog</button>
      </div>
    </div>
  );
};
